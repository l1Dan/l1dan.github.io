...
## 属性
在处理 *state* 而不是 *behaviour* 时，更加倾向于使用属性而不是方法。
...
- 对于 `init` 方法，我们应该使用 Objective-C 2.0 规范，因此请在 `init` 方法中 ***始终使用 instancetype*** 作为返回值类型。
- 对于工厂方法，有两种情况，这两种方式能够很好的表达便利构造函数。
    - 当工厂方法可以被子类化时: ***使用 instancetype***
    - 当工厂方法不打算被子类化时: ***使用显示类型***
